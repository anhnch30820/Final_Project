...
def get_final_mask_with_topCRF(coco_class, img_id, mask_dir, img, th_fg):
    return_dict = {}
    anno_ids = coco_class.getAnnIds(imgIds=img_id)
    annotations = coco_class.loadAnns(anno_ids)
    labels = []
    for i in annotations:
        labels.append(i['category_id'])
    # anno = _preprocess_annotation(anno, img_id)
